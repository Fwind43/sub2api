...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	modelPlaygroundTaskRepository := repository.NewModelPlaygroundTaskRepository(redisClient)
	modelPlaygroundHandler := handler.NewModelPlaygroundHandler(apiKeyService, modelPlaygroundTaskRepository, configConfig)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlaygroundHandler, idempotencyCoordinator, idempotencyCleanupService)
